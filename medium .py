...
class ComputerAi(GameFuctions):
    ...
    def convert_positions(self, index:list, keys=False, values=False)->list:
        if keys:
            key =[k for k, v in self.map_get_position.items() for ind in index if ind == v ]
            return key
        if values:
            value = [v for k, v in self.map_get_position.items() for m in index if k == m]
            return value
    ...
    def computer_player(self):
        """
        Computer player (HARD mode just it has learned the game)
        """
